...
public class DouyinParser {
    public static void get(String data,final DownloadCallback downloadCallback) {
        List<String> listUrl = FindUrlInString.parse(data);
        if (listUrl.size() == 0) {
            downloadCallback.fail("请输入含有正确url的字符串");
            return;
        }
        Light.getRequest(listUrl.get(0)).asyncExecute(new AbstractCallback() {
            @Override
...
//                    List<String> list = FindJsonInString.parse(body);
                    if (douyinBean != null) {
                        ...
                        DowloadDouyin.download(videoid,downloadCallback);
                    }
                }
            }
...
//        OkHttpUtil.get(data,new Callback() {
//                    @Override
...
//                        if (response.isSuccessful()) {
//                            System.out.println(response.request().data());
